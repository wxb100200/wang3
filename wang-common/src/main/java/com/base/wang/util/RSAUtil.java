...
//    }
     ...
     * 打印密钥对
     */
    public static Map<String,String> generateKeyPairVo(){
        try{
            KeyPairGenerator keyPairGen = KeyPairGenerator.getInstance(KEY_ALGORITHM);
            // 密钥位数
            keyPairGen.initialize(2048);
     ...
     * 用私钥对信息生成数字签名
     */
    public static String sign(String json,String privateKey)  {
        try {
            ...
            return StringUtil.bytesToHexString(sign);
        }catch (Exception e){
            e.printStackTrace();
            return null;
        }
    ...
    public static boolean verify(String data, String sign,String publicKey) {
        try {
            ...
            return signature.verify(StringUtil.hexStringToBytes(sign));
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
